...
public class CandidateDao implements AutoCloseable {
	...
	// add new candidate
	    public int saveUser(Candidate c) throws Exception{
		...
		// update candidate
		public int updateUser(Candidate c) throws Exception{
		...
		public List<PartyVotes> getPartywiseVotes() throws Exception {
			...
			String sql = "SELECT party,SUM(votes) as SUMVOTE FROM candidates GROUP BY party";
			try(PreparedStatement stmt = con.prepareStatement(sql)){
				try(ResultSet rs = stmt.executeQuery()){
					while(rs.next()) {
						String party = rs.getString("party");
						int votes = rs.getInt("SUMVOTE");
